...
![Build Status](https://img.shields.io/badge/CI%2FCD%20Pipeline-PASSED%20(108%2F108%20Tests)-02c39a?style=for-the-badge&logo=githubactions)
...
- 🧾 **GST Tax Invoice & Email Dispatch Engine API**: `POST /api/admin/send-tax-invoice-email/` (Automated GST tax invoice breakdown and real SMTP email delivery)
- ⏱️ **Doctor Queue Wait-Time Estimator Engine API**: `POST /api/admin/queue-wait-time/` (Dynamic patient wait-time prediction based on live queue positions and triage priority)
- 📜 **AI Clinical Digital Prescription Generator & Verification API**: `POST /api/admin/generate-digital-rx/` (Cryptographic HMAC-SHA256 e-prescriptions with QR code validation)
- 🛡️ **Patient Drug-Allergy & Cross-Reaction Safety Shield API**: `POST /api/admin/drug-allergy-shield/` (Beta-lactam & NSAID cross-reactivity and contraindication safety audits)
- 📞 **Twilio IVR Interactive Voice Call & Emergency Audio Dispatcher API**: `POST /api/admin/trigger-voice-call/` (Automated IVR voice calls to patient mobile numbers for OPD token callouts and emergency alerts)
